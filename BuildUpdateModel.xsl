...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="d">{
        <xsl:for-each select="EntityType"> <xsl:if test="./Property/@updatable = 'true'">
            "Update<xsl:value-of select="./@Name"/>": {
            "id": "Update<xsl:value-of select="./@Name"/>",

            "properties" : {
            <xsl:if test="./Property/@updatable = 'true'">
                <xsl:apply-templates select="./Property"/></xsl:if>
            <xsl:if test="./NavigationProperty/@updatable = 'true'">
    ...
    <xsl:template match="EntityType/Property">
        <xsl:if test="./@updatable = 'true'">
            ...
            <xsl:choose> <xsl:when test="count(preceding-sibling::*[@updatable= 'true'])&gt; 0">,
            </xsl:when> <xsl:otherwise>
            </xsl:otherwise>
            </xsl:choose>
